...
* **`cli.rs`** — argv parsing for headless subcommands (`--run "Name" "Exe"`, `--backup "Name"`, `--release-lock "Name"`) vs normal launch. Used at startup and by the single-instance forwarding callback.
...
* **RAII run-lock**: `runner.rs` acquires a single-launch guard whose `Drop` releases the slot (even on panic — else a crashed workflow wedges all launches until restart). **In-process only** (a `Mutex` in `RunState`); it does nothing across the several Spool processes on a machine — that's the backup lock's job.
* **Cross-process backup lock** (`proc_lock.rs`): several Spool processes run at once on one machine (tray GUI, attached `spool --run`, Decky `--headless-server`, one-shot `--backup` fallbacks), and ludusavi's backup dir + the rclone remote folder are one shared tree — two running `ludusavi backup` / `cloud upload` at once can corrupt the backup or last-writer-win the remote. (The DB is already safe via WAL; this guards the side effects it can't.) `acquire_backup()` takes a machine-wide advisory file lock (`std::fs::File` native `lock`/`try_lock`/`unlock`) on `backup.lock`; the OS frees it on exit, so a crash can't wedge it. Held across local backup **and** cloud upload so the pair is atomic; taken at `backup_game_core` (headless `--backup` + manual "back up now") and the play workflow's backup/upload span. A single **global** lock (the remote is one tree). Acquisition is **fail-safe**: taken *before* any ludusavi/rclone work, polls with a timeout; on timeout the caller gives up rather than run unlocked (the live save is on local disk regardless, so deferring is safe — and a timeout means nothing was half-written and another process is already backing up). Manual "back up now" surfaces a retryable error toast; headless `--backup` exits non-zero; the play workflow soft-defers (records playtime, sets `local-newer` badge, leaves the `PendingBackup` marker, lets the next launch reconcile). Polling (not blocking `lock()`) keeps the bounded timeout and degrades same-process re-entry to a wait, never a deadlock.
