...
* **`library.rs`** — `GameEntry` + `Library` CRUD, backed by a **SQLite database** (`library.db`) via [`sqlx`](https://github.com/launchbadge/sqlx) so multiple Spool processes (the tray GUI, the attached `spool --run` launch, the headless Decky server) can write the library concurrently without the old whole-document JSON rewrites clobbering each other. Each game is one row: `id` / `catalog_number` / `game_name` columns plus a `data` column holding the full `GameEntry` as JSON — so adding a `GameEntry` field needs **no schema migration** (old rows still parse via `serde(default)`, exactly as `library.json` did). Reads deserialise `data`; targeted writes use SQLite `json_set()` to update individual fields atomically (`bump_session`, `record_backup_stats`, `set_sync_badge`, `set_art`, `update_fields`, …), so e.g. an attached-launch playtime bump can't overwrite a concurrent editor save. The whole-entry `replace` (the editor's save) re-overlays the workflow-owned runtime fields (playtime, backup stats, sync badges, art/size) from the existing row so it can never clobber them. A `meta.version` counter (bumped by a trigger on every games mutation) is the **cross-process change signal**: the GUI polls it (`spawn_library_change_poll` in `lib.rs`) and re-emits `library:changed` when another process writes, since Tauri events don't cross process boundaries. On first run the legacy `library.json` is imported once and renamed to `library.json.migrated` (kept as a rollback backup). Queries are runtime-checked (`sqlx::query`/`query_as`), so there's no compile-time `DATABASE_URL` or `.sqlx` offline cache and CI builds unchanged. `WAL` journal mode + a `busy_timeout` give the concurrent access. Emits `library:changed` on every in-process mutation.
...
* **`runner.rs`** — the marquee feature. State machine: `restoring → launching → playing → backing-up → uploading → done`, emitting `run:phase` events at each transition (consumed by the in-app UI, the Game-Mode splash window, and — when the main window is hidden — native OS notifications). The post-session backup is split into a local-write step (`backing-up`, `ludusavi backup --api --force`) and a separate cloud step (`uploading`, `ludusavi cloud upload --api --force`, emitted only when a remote is configured) so the splash can show a live upload spinner instead of jumping from the local backup straight to `done`. Post-play, local is authoritative, so the upload force-overwrites the remote (the same resolution the old combined `backup --cloud-sync` call applied on a conflict). Single-launch RAII guard releases the slot even on panic. Cloud restore pulls from the remote first; a true local-vs-cloud divergence emits a `cloud:notice`/conflict signal that the frontend's `CloudConflictModal` resolves, while a cloud copy that's merely newer is fast-forwarded silently. Backup/upload failures after a successful session log-and-continue.
...
* **Lock discipline**: never hold a `std::sync::Mutex` guard across `.await`. Every async command snapshots what it needs from state, drops the guard, then awaits. If state must cross an await point, that specific state moves to `tokio::sync::Mutex`. (The library is the exception — it's a `sqlx::SqlitePool`, async-native, so its `SharedLibrary` is `Arc<Library>` with no outer `Mutex`.)
* **SQLite library, multi-process safe**: the game library is a SQLite DB (`library.rs`), not a JSON file, because several Spool processes write it concurrently. WAL mode serialises writers; targeted `json_set()` field updates and a runtime-field-preserving `replace` mean concurrent writes touch different fields instead of clobbering one shared blob (see the `library.rs` bullet above). `config.json` is *still* atomic-JSON (small, single-writer): `config.rs` writes a temp file then `rename`s over the target.
...
* **RAII run-lock**: `runner.rs` acquires a single-launch guard whose `Drop` impl releases the slot. Releases on panic too — without it a crashed workflow would leave Spool unable to launch any game until restart. This guard is **in-process only** (a `Mutex` in `RunState`); it serialises launches within one Spool process but does nothing across the several Spool processes that share a machine — that's what the backup lock below is for.
* **Cross-process backup lock** (`proc_lock.rs`): several Spool processes run at once on one machine (the tray GUI, an attached `spool --run`, the Decky `spool --headless-server`, one-shot `spool --backup` fallbacks), and ludusavi's backup dir plus the rclone remote folder are a single shared tree — two of them running `ludusavi backup` / `cloud upload` at once can corrupt the backup or last-writer-win on the remote and lose a save. (The *database* is already safe — SQLite WAL; this guards the side effects it can't.) `acquire_backup()` takes a machine-wide advisory file lock (`std::fs::File` native `lock`/`try_lock`/`unlock` — `flock`/`LockFileEx`) on `backup.lock` in the app dir; the OS frees it when the holder exits, so a crash or Steam force-kill can't wedge it. Held across the local backup **and** the cloud upload so the pair is atomic versus another process; taken at two chokepoints — `backup_game_core` (covers headless `--backup` + the manual "back up now" command) and the play workflow's backup/upload span. It's a single **global** lock (the remote folder is one shared tree, so per-game wouldn't help) and acquisition is **fail-safe**: it's taken *before* any ludusavi/rclone work and polls with a timeout; on timeout the caller gives up rather than run unlocked — a concurrent write could corrupt the backup or clobber the remote, and the live save is on local disk regardless, so deferring and retrying is safe. A timeout also means nothing was written (nothing half-finished to reconcile) and that another process holds the lock — i.e. is already backing up. The manual "back up now" command surfaces the timeout as a retryable error toast and headless `--backup` as a non-zero exit; the play workflow treats it as a soft defer (records playtime, sets the `local-newer` badge, leaves the `PendingBackup` marker so peers keep warning, and lets the next launch reconcile). Polling rather than a blocking `lock()` keeps the bounded timeout and degrades the rare same-process re-entry to a wait, never a deadlock.
...
* **JSON shape compatibility**: `config.json` and the library's per-row `data` JSON both round-trip cleanly because `ConfigData` / `GameEntry` carry a **container-level** `#[serde(default)]`, so missing keys fall back to the struct's `Default` and adding fields never breaks older data — and for the library this means a new `GameEntry` field needs *no* SQLite schema migration. Apply `#[serde(default)]` at the struct (container) level, not per-field — a per-field default shadows the struct's custom `Default` values with the field-type default. Fields the app no longer uses are removed, not kept around for legacy round-trip.
