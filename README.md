...
2. Check that no other device is already playing the game (if sync server is configured)
...
Games are stored in `%LOCALAPPDATA%\ludusavi-wrap\library.json` with atomic writes to prevent corruption.
...
- When LAN Share is enabled in Settings, the app starts a lightweight HTTP file server on your local network (default port 47632) and broadcasts its presence via UDP.
...
- The app guards against duplicate downloads: if the game is already in your library, re-downloading is blocked until you remove it first.
...
ludusavi-wrap can connect to a sync server to coordinate saves across devices. Configure a server URL and API key in Settings under the **Sync Server** card.

**Cross-device play state lock:**

Before launching a game, the app acquires a lock on the sync server. If another device is already playing that game, you'll be warned and the launch is blocked — preventing the classic "last device to close wins" save conflict. The lock is released automatically when the game closes. Stale locks (from a crash or lost connection) are detected and can be overridden.
...
[TorBox](https://torbox.app) is a torrent debrid service — it downloads torrents server-side and lets you pull the result at full speed. ludusavi-wrap integrates directly:

- Configure your TorBox API key and a local download directory in Settings.
- When downloading a game via the **Browse Games** window (see below), the app can send the torrent/magnet to TorBox and then download the cached result to your machine.
...
Open Settings → **Download Sources** — add one or more URLs pointing to Hydra-format JSON catalogues. The app fetches and merges them on each open.

### Windows Toast Notifications

Game sessions no longer show a blocking progress window. Instead, the app hides itself and uses **Windows toast notifications** to communicate:
...
When a new version is available, the app shows a simple yes/no prompt. Accepting downloads and silently installs the update in the background — no UAC prompts or wizard windows.
