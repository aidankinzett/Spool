...
**Extract a ludusavi-prep helper for the resolve/`ensure_config`/`config_dir` boilerplate — scoped to four handlers**
...
**Harden the appid coupling: dedupe the computation + add a cross-helper test, plus a name/single-record fallback** *(synthesizing the worthwhile parts of a proposal whose Part B was rejected)*
