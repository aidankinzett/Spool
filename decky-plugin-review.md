...
**~~Forced-close fallback overwrites a peer's live session marker (missing ownership guard)~~ ✓ Done — PR #373**
...
**~~Unguarded `findClassModule` access crashes the game-detail patch when Steam renames CSS-module markers~~ ✓ Done — PR #421** *(refined to High here despite both verifiers landing on Medium — see rationale)*
...
**~~LAN peer-proxy endpoints issue outbound HTTP to an arbitrary caller-supplied host:port with no allow-list~~ ✓ Done — PR #422** *(reviewers split medium/low; medium retained — see note)*
...
**~~Forced-close fallback silently skipped when the library DB fails to open~~ ✓ Done — PR #423**
`tauri/src-tauri/src/plugin_server/mod.rs, session.rs, saves.rs, library.rs, proton.rs, steam.rs`
...
**~~`get_steam_art`: "capsule" kind never resolves via CDN/SteamGridDB fallback~~ ✓ Done — PR #426** *(reviewers split medium/low)*
...
**~~`get_steam_art`: failed WebP transcode sends raw WebP bytes mislabeled as PNG~~ ✓ Resolved by the plugin_server refactor (PR #405)**
`tauri/src-tauri/src/plugin_server/steam.rs:158-184`
Verified during Group 2: `transcode_webp_to_png` now returns `None` on both decode and encode failure, and `get_steam_art` treats `None` as a 404 (no fall-through to the non-WebP branch). Undecoded WebP can no longer be labeled as PNG. No further change needed.
...
**~~Active-session record has no cross-process file lock (lost-update window)~~ ✓ Done — Group 2**
...
**~~`session.rs` cross-process serialization comment is inaccurate during the Game-Mode backup phase~~ ✓ Done — Group 2** *(reviewers split low/nit)*
`tauri/src-tauri/src/session.rs:16-24`
Resolved together with the lost-update finding above: rather than just correcting the comment, the read-modify-write now takes a machine-wide advisory file lock (`active-session.lock`) on top of the in-process mutex, so the cross-process serialization the comment describes is actually enforced. Comment updated to match.
...
**~~"Back up now" (plugin server) backs up whatever the record names, with no appid/game validation~~ ✓ Done — Group 2**
...
**~~Malformed/partial response on first attempt re-fires a non-idempotent POST~~ ✓ Done — Group 3** *(reviewers split medium/low)*
...
**~~Concurrent callers can double-spawn the headless server (orphaned ephemeral-port server)~~ ✓ Done — Group 3**
`decky/main.py:74-99`
...
**~~`settings.json` written non-atomically~~ ✓ Done — Group 3**
...
**~~`POST /fold` has no server-side in-flight guard (overlapping rclone folds)~~ ✓ Done — Group 2** *(accepted from coverage-gap investigation, 2026-06-09)*
