...
fn main() {
    ...
    // That file is a `pnpm build` artifact and is gitignored, so on a fresh
    ...
    // `pnpm build` first so the genuine bundle is what actually gets embedded.
    //
    ...
    if std::env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("linux") {
        ...
        if !index.exists() {
            ...
            let _ = std::fs::write(
                &index,
                "// placeholder — run `pnpm build` in decky/ to embed the real plugin bundle\n",
            );
        }
