...
pub async fn pull_cloud_saves_core(
    ...
    if !conflict {
        ...
        if let Some(tip) = redirects::read_local_backup_tip(&backup_dir, &game_name) {
            let _ = set_baseline_in(library, game_id, &tip.name);
        }
        mark_synced_badge(library, game_id);
...
fn mark_synced_badge(library: &SharedLibrary, game_id: &str) {
...
fn set_cloud_baseline(app: &AppHandle, game_id: &str, tip_name: &str) -> AppResult<()> {
    let library = app.state::<SharedLibrary>();
    set_baseline_in(&library, game_id, tip_name)
}
...
/// plugin server).
fn set_baseline_in(library: &SharedLibrary, game_id: &str, tip_name: &str) -> AppResult<()> {
    let mut lib = library.lock().map_err(|_| AppError::LockPoisoned)?;
    if let Some(entry) = lib.entries.iter_mut().find(|e| e.id == game_id) {
