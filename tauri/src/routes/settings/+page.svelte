        ...
        <div class="mx-auto max-w-[720px]">
              ...
              <div id="cloud-saves">
                <SettingsCard title="Cloud saves (rclone)" helper="Configure a cloud remote here, then use 'Open Ludusavi settings' to run rclone config / authenticate.">
                  ...
                  {/if}
                    ...
                    <SettingsRow label="Remote folder" helper="Saves go to <folder>/ludusavi-backup; Spool metadata to <folder>/_spool">
                      {#snippet control()}
          ...
          <section class="mb-9">
            ...
            <div class="flex flex-col gap-4">
              ...
              <div id="sync">
                <SettingsCard title="Cloud reachability" helper="Spool probes your configured rclone remote every 30 s to check whether it's reachable.">
                  <div class="flex items-center gap-[14px] px-[18px] py-[14px]">
                    <div class="flex-1">
                      <div class="flex items-center gap-2 text-[13px] font-medium text-ink-0">
                        Remote status
                        <Pill kind={syncStatus.reachability === 'online' ? 'ok' : syncStatus.reachability === 'offline' ? 'warn' : 'off'}>
                          {syncStatus.reachability === 'online' ? 'Online' : syncStatus.reachability === 'offline' ? 'Offline' : 'Unconfigured'}
                        </Pill>
                      </div>
                      <div class="mt-[3px] text-[11.5px] text-ink-2">
                        {#if syncStatus.reachability === 'online'}
                          Remote reachable — session markers and device blobs will sync.
                        {:else if syncStatus.reachability === 'offline'}
                          Remote unreachable · {syncStatus.error ?? 'no response'} — launches will proceed without cross-device warning.
                        {:else}
                          No cloud remote configured — session coordination disabled.
                        {/if}
                      </div>
                    </div>
